import { Bell, Crosshair, LocateFixed, MapPin, Navigation, Route, Search, Star, Target } from "lucide-react";
import { DeviceFrame, SubHeader } from "../components/DeviceFrame";
import { BottomNav, Button, LocationBadge, type BottomNavKey } from "../components/UI";
...
const mapSpots = [
  { name: "인사동 전통찻집", area: "종로구 인사동", point: "240P", className: "pin-a" },
  { name: "북촌 골목 산책", area: "북촌 한옥마을", point: "180P", className: "pin-b" },
  { name: "청계천 포토존", area: "청계광장", point: "160P", className: "pin-c" },
];

export function MapPage({ onBack, onNavigate, onStartMission }: MapPageProps) {
  return (
    <DeviceFrame className="app-device">
      ...
      <section className="map-screen">
            ...
            {mapSpots.map((spot, index) => (
              <article key={spot.name}>
                <span className={`nearby-icon ${spot.className}`}>
                  {index === 0 ? <MapPin size={18} strokeWidth={2.5} /> : index === 1 ? <Route size={18} strokeWidth={2.5} /> : <Navigation size={18} strokeWidth={2.5} />}
                </span>
                <div>
        ...
        <Button className="home-main-cta" icon={<Navigation size={18} strokeWidth={2.4} />} onClick={onStartMission}>
          선택한 미션 시작하기
        </Button>
      </section>
