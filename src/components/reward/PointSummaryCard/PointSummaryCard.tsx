import PointSummaryCardSkeleton from "./PointSummaryCardSkeleton";
...
export default function PointSummaryCard({
  ...
  return (
    <section
      className="mt-[44px] rounded-[16px] border border-white/80 bg-white/70 p-[16px] shadow-[0_1px_3px_rgba(0,0,0,0.12)] backdrop-blur"
      aria-busy={isLoading}
    >
      <p className="m-0 text-[13px] font-medium leading-[18px] text-[#A2A9B2]">
        보유 포인트
      </p>
      {isLoading && <PointSummaryCardSkeleton />}
      {!isLoading && isError && (
        <RewardError
          message="포인트를 불러오지 못했어요."
        />
      )}
      {hasData && !isLoading && (
        <>
